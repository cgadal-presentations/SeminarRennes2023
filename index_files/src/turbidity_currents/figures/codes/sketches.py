...
slope = -7
water_height_fact = 0.89

figsize = (0.8 * quarto.regular_fig_width, 0.55 * quarto.regular_fig_height)
...
#
xy = bottom_left - 0.1 * door_height * slope_vec_up
xytext = bottom_door - 0.1 * door_height * slope_vec_up
ax.annotate("", xytext=xytext, xy=xy, arrowprops=dict(arrowstyle="<->", shrinkA=0, shrinkB=0, color="k"))
xytext = (xy + xytext) / 2
...
ax.text(0.7, 0.3, r"$\rho_{\rm a}$", color="k", ha="left", va="bottom", transform=ax.transAxes)

ax.text(-0, 0.425, r"$\rho_{\rm f}$", color="k", ha="right", va="bottom", transform=ax.transAxes)

ax.text(-0, 0.325, r"$\rho_{\rm s}$, $d$, $\phi$", color="k", ha="right", va="bottom", transform=ax.transAxes)

figname = "../{}_general.svg".format(sys.argv[0].split(os.sep)[-1].replace(".py", ""))
