...
from color_template_dune import color_BI, color_F, linewidth_barscale, lw_arrow
from matplotlib.colors import to_rgba

path_gen = "/media/cyril/Backup_dune/DUNE/PhD_Parts/Part5_Winds/Giant_dune_retroaction_regional_wind_regime"
...
def North_arrow(fig, ax, center, length, length_small, width, radius, theta=0, color="k"):
    ...
    arrow = np.array(
        [[0, -y_start], [width / 2, -radius], [0, -(radius + length)], [-width / 2, -radius], [0, -y_start]]
    )
...
Orientations = np.array([Data[station]["Orientation_insitu"], Data[station]["Orientation_era"]])
...
# sediment fluxes
q = np.array(
    [quadratic_transport_law(theta, theta_th_quadratic, Omega), quartic_transport_law(theta, theta_th_quartic)]
)
...
alpha_BI = Bed_Instability_Orientation(
    Angles[None, None, None, None, :], PDF[None, :, :, :, :], gamma=gamma[:, None, None, None, None]
)
...
left_point = 0
aspect_ratio = 2.3
path_im = "/media/cyril/Backup_dune/DUNE/PhD_Parts/Thèse/Chapitres/chapter5/figures/Figure_small_scale_finger/"
...
for j in range(7):
    # Figure
    figsize = (quarto.regular_fig_width, quarto.regular_fig_width / aspect_ratio)
    ...
    # échelle
    backgrnd = ptch.Rectangle((0, 0), width=0.16, height=0.11, transform=ax.transAxes, color="w", alpha=0.6)
    ...
    if j > 0:
        ...
        if j > 1:
            ...
            if j > 2:
                # small fingers
                ellispe4 = ptch.Ellipse((277, 533), 400, 40, angle=-35, color=color_F, fill=False, linestyle="--")
                ax.add_artist(ellispe4)
                ellispe5 = ptch.Ellipse((1323, 500), 350, 50, angle=-35, color=color_F, fill=False, linestyle="--")
                ax.add_artist(ellispe5)

    # flux rose era
    if j > 3:
        #
        anchor = [-0.25, 0.64, 0.45, 0.45]
        subax = ax.inset_axes(bounds=anchor, transform=ax.transAxes)
        a = plot_flux_rose(Angles, PDF[0, index, 1, :], subax, fig, cmap=theme.flux_color, edgecolor="k", linewidth=0.5)
        #
    ...
    if j > 5:
        ...
        plot_arrow(
            ...
            "centered",
            arrowprops=dict(
                arrowstyle="<|-|>", color=color_BI, shrinkA=0, shrinkB=0, lw=lw_arrow, mutation_scale=10, linestyle="-"
            ),
        )
        plot_arrow(
            ax,
            ...
            "not_centered",
            arrowprops=dict(
                arrowstyle="<|-", color=color_F, shrinkA=0, shrinkB=0, lw=lw_arrow, mutation_scale=10, linestyle="-"
            ),
        )
        plot_orientation_wedge(
            ax, alpha_F[:, :, :, 1], alpha_BI[:, :, :, 1], center, length, color_F, color_BI, alpha=0.2
        )

    if j > 4:
        # flux rose station
        anchor = [0.77, 0.64, 0.45, 0.45]
        subax = ax.inset_axes(bounds=anchor, transform=ax.transAxes)
        a = plot_flux_rose(Angles, PDF[0, index, 0, :], subax, fig, cmap=theme.flux_color, edgecolor="k", linewidth=0.5)
        #
    ...
    if j == 5:
        anchor = [0.45, 0.1, 0.45, 0.65]
        subax = ax.inset_axes(bounds=anchor, transform=ax.transAxes)
        path_img_station = (
            "/media/cyril/Backup_dune/DUNE/PhD_Parts/Part5_Winds/Namibia/Giles_data/Work/Deep_Sea_Station/image002.png"
        )
        img_station = np.array(plt.imread(path_img_station))
        subax.imshow(img_station)
        subax.set_xticks([])
        subax.set_yticks([])
        el = ptch.Ellipse((2, -1), 0.5, 0.5)
        subax.annotate(
            "",
            xy=(-1, 1.1),
            xytext=(0, 0.5),
    ...
    if j > 5:
        ...
        plot_orientation_wedge(
            ax, alpha_F[:, :, :, 0], alpha_BI[:, :, :, 0], center, length, color_F, color_BI, alpha=0.2, linestyle="--"
        )
    ...
    #
    backgrnd = ptch.Rectangle((0.95, 0), width=0.05, height=0.2, transform=ax.transAxes, color="w", alpha=0.6)
    ax.add_patch(backgrnd)
    North_arrow(fig, ax, center, length, length_small, width, radius, theta=0, color="k")
    #
    # %% saving figure
    figname = "../{}_{}.svg".format(sys.argv[0].split(os.sep)[-1].replace(".py", ""), j)
