...
from color_template_dune import color_BI, color_max, flow_color
...
#
# ####
ax_sub = ax.inset_axes([0.1, 0.025, 0.40, 0.50])
ax_sub.set_xticks([])
ax_sub.set_yticks([])
path = "/media/cyril/Backup_dune/DUNE/PhD_Parts/Thèse/Chapitres/chapter3/figures/Figure1_field/Images/Angola/"
img = np.array(plt.imread(path + "Zone.png"))
ax_sub.imshow(img[120:820, :])
#
# flow orientation
xy = np.array([146, 595])
xytext = np.array([301, 452])
ax_sub.annotate("", xy, xytext, arrowprops=dict(arrowstyle="<|-", shrinkA=0, shrinkB=0, color=flow_color))
# Dune orientation
xy = np.array([421, 125])
xytext = np.array([673, 407])
ax_sub.annotate("", xy, xytext, arrowprops=dict(arrowstyle="<|-|>", shrinkA=0, shrinkB=0, color=color_BI))

ax.set_xlabel(r"Wavenumber, $k = 2\pi/\lambda$")
ax.set_ylabel(r"Growth rate, $\sigma$")
ax.set_xlim([0, 0.9])
ax.set_ylim(bottom=-0.013, top=0.016)
ax.set_xticks([xmax, xzero])
ax.set_xticklabels([r"$\lambda_{\rm max}$", r"$\lambda_{\rm c}$"])
ax.set_yticks([0, ymax])
ax.set_yticklabels(["0", r"$\sigma_{\rm max}$"])

plt.setp(ax.get_xticklabels()[0], color=color_max)
plt.setp(ax.get_xticklabels()[1], color="tab:red")
plt.setp(ax.get_yticklabels()[1], color=color_max)
